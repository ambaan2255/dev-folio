...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ projectId, onBack }) => {
  ...
  useEffect(() => {
    if (project) {
      document.title = `${project.title} | Maurice Gift`;
        ...
        <SectionWrapper delay={300} className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 py-6 md:py-12">
            ...
            <div className="flex flex-wrap gap-3 md:gap-4">
              {project.techStack.map(tech => (
                <div key={tech} className="flex flex-col items-center gap-2 group/icon">
                  <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border-2 border-transparent group-hover/icon:border-indigo-500 transition-all">
                    <TechIcon name={tech} />
                  </div>
          ...
          <div className="bg-white dark:bg-gray-900 p-6 md:p-8 rounded-2xl md:rounded-[2rem] border-2 border-gray-100 dark:border-gray-800 shadow-xl space-y-4 md:space-y-6">
            ...
            <div className="flex flex-wrap gap-3 md:gap-4 pt-2">
              ...
              {project.liveUrl && <a href={project.liveUrl} title="Live Preview" target="_blank" rel="noopener noreferrer" className="p-3 md:p-4 bg-indigo-600 text-white rounded-xl md:rounded-2xl hover:scale-110 transition-all shadow-lg md:shadow-xl shadow-indigo-600/30"><svg className="w-5 h-5 md:w-6 md:h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg></a>}
